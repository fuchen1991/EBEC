...
public class NaiveBisimulation {

	private NFABase nfa;
	private boolean[][] rel;
	
	public NaiveBisimulation(NFABase nfa)
	{
		this.nfa = nfa;
	}
	...
	public void computeBisimulation()
	{
		...
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<n;j++)
			{
				...
				{
					if((nfa.getAcceptStateSet().get(i) && !nfa.getAcceptStateSet().get(j)) || (!nfa.getAcceptStateSet().get(i) && nfa.getAcceptStateSet().get(j)))
						rel[i][j] = false;
					else
						rel[i][j] = true;
				}
